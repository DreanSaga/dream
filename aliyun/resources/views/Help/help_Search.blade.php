...
<html>
  ...
  <body>
    ...
    <div class="x-body">
      <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so layui-form-pane" method="post" action="{{url("Help_search")}}">
          <input class="layui-input" placeholder="请输入分类名称" name="cate_name">
          <button class="layui-btn" >搜索</button>
          <a class="layui-btn" href="{{url('Help_show')}}">返回列表</a>
        </form>
      </div>
      <blockquote class="layui-elem-quote">直播分类管理</blockquote>
      <xblock>
        <button class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon"></i>批量删除</button>
        <a class="layui-btn" href="{{url('Help_Add')}}"><i class="layui-icon"></i>增加</a>
        ...
        <tbody class="x-cate">
         @foreach($info as $k =>$v)
          <tr cate-id='5' fid='0' >
            ...
            <td class="td-manage">
              <a href="{{url('Help_Edit',['id'=>"$v->id"])}}" class="layui-btn layui-btn-warm layui-btn-xs">编辑</a>
              ...
              <a href="{{url('Help_Del',['id'=>"$v->id"])}}" class="layui-btn layui-btn-warm layui-btn-xs">删除</a>
            </td>
          </tr>
           @endforeach
        </tbody>
